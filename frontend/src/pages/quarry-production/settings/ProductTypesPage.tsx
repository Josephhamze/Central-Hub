...
import { productTypesApi, type ProductType } from '@services/quarry-production/settings';
...
export function ProductTypesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['product-types', search, isActiveFilter],
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; isActive?: boolean }) => productTypesApi.create(data),
    onSuccess: () => {
    ...
    onError: (err: any) => showError(err.response?.data?.error?.message || 'Failed to create product type'),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => showError(err.response?.data?.error?.message || 'Failed to update product type'),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => showError(err.response?.data?.error?.message || 'Failed to delete product type'),
  });

  const handleEdit = (productType: ProductType) => {
    setEditingId(productType.id);
    setFormData({ name: productType.name, isActive: productType.isActive });
  ...
  return (
    <PageContainer
      title="Product Types"
      description="Manage finished product types"
      actions={
        canCreate ? (
          <Button
            variant="primary"
    ...
    >
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {data?.data.data.items.map((productType) => (
                  <tr key={productType.id} className="border-b border-border-default hover:bg-bg-hover">
                    <td className="p-4 font-medium text-content-primary">{productType.name}</td>
                    <td className="p-4">
                      <Badge variant={productType.isActive ? 'success' : 'default'}>
            ...
            {data?.data.data.items.length === 0 && (
              <div className="p-8 text-center text-content-secondary">No product types found</div>
            )}
          </div>
        )}
      </Card>
      ...
      <Modal
        ...
        title={editingId ? 'Edit Product Type' : 'Create Product Type'}
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="e.g., 20mm Aggregate, 40mm Aggregate, Dust"
            required
          />
          <div>
